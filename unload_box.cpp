...
int main(int argc, char** argv) {
    ...
    // Declaring a part variable for use later
	...
	// compare the orphaned part coordinates, within range of 0.01, to the current part position. 
	float current_part_position_x = current_part.pose.pose.position.x;
	float current_part_position_y = current_part.pose.pose.position.y;
	float current_part_position_z = current_part.pose.pose.position.z;

	ROS_INFO("Bad part (x, y, z) position: (%f, %f, %f)", current_part_position_x, current_part_position_y, current_part_position_z);

	// go through each orphaned model and compare to the current part
	bool bad_part_found = false;
	// int loc_of_bad_part;
	for (int i = 0; i < nparts && !bad_part_found; i++) {
		float orphaned_part_position_x = orphan_models_wrt_world[i].pose.position.x;
		float orphaned_part_position_y = orphan_models_wrt_world[i].pose.position.y;
		float orphaned_part_position_z = orphan_models_wrt_world[i].pose.position.z;

		ROS_INFO("Comparing bad part position with orphaned part with position: (%f, %f, %f)", orphaned_part_position_x, orphaned_part_position_y, orphaned_part_position_z);

		if ( (current_part_position_x-0.05 < orphaned_part_position_x) && (orphaned_part_position_x < current_part_position_x+0.05) && (current_part_position_y-0.05 < orphaned_part_position_y) && (orphaned_part_position_y < current_part_position_y+0.05) && (current_part_position_z-0.05 < orphaned_part_position_z) && (orphaned_part_position_z < current_part_position_z+0.05) ) {
			// part found, print name of the part. 
			model_to_part(orphan_models_wrt_world[i], bad_part, inventory_msgs::Part::QUALITY_SENSOR_1);
			ROS_INFO("Defective part type found: %s", bad_part.name);
			bad_part_found = true;
			// model_to_part(orphan_models_wrt_world[i], bad_part, 1);
			// loc_of_bad_part = i;
		}

		else if ( i == nparts - 1 ) {
			ROS_INFO("Defective part's type could not be found!");
		}
	}
	...
	// First,  remove the defective part. 
	robotBehaviorInterface.pick_part_from_box(bad_part, 50.0);
	robotBehaviorInterface.discard_grasped_part(bad_part, 50.0);
	ROS_INFO("Defective part removed!");
	...
	// For each part in the box, remove it. 
	for (int i = 0; i < nparts; i++) {
		inventory_msgs::Part part_to_pick;
		model_to_part(orphan_models_wrt_world[i], part_to_pick, inventory_msgs::Part::QUALITY_SENSOR_1);
		robotBehaviorInterface.pick_part_from_box(part_to_pick, 50.0);
		robotBehaviorInterface.discard_grasped_part(part_to_pick, 50.0);
	}
